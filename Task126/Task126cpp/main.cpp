...
unsigned int number_of_possible_options(std::vector<char> source_alphabet, unsigned int lenght_option, std::string except_substring) 
{
	...
	auto p = calculacte_p_Bernoulli(lenght_option, except_substring.length(),
		static_cast<double>(lenght_option - except_substring.length()) / source_alphabet.size()) * all_option;
